...
&emsp;&emsp;这里介绍如图1-16所示的INT8线性对称量化方法：首先找出每个网络层中参数取值区间端点绝对值的最大值MAX，然后将网络参数所在的[-MAX, MAX]的区间线性映射到[-127, 127]。例如，假设某个网络层的参数在[-3, 6]的范围内，则MAX取值为6，并将[-6, 6]线性映射到[-127, 127]；此时，6用127表示，-3用-63表示，如此便可将网络参数的大小缩小75%。
...
&emsp;&emsp;线性对称的量化方法适用于网络参数关于原点对称的场景，否则，量化后的数据将存在较为严重的精度丢失。此时，为了将参数更均匀地映射到量化区间，以获得更好的量化效果，可以根据网络参数的分布情况对量化区间的端点进行调整，如图1-17所示。

<center><img src="../assets/1-17.jpg" width = 280></center>
<center>图1-17 线性非对称量化方法图示</center>

&emsp;&emsp;图1-17是在图1-16的基础上，截断左侧部分参数之后所得到的INT8非线性对称量化方法。此时，落在图中[-T, T]区间外的参数将使用区间端点所对应的定点数来表示。例如，假设网络参数的取值范围是[-3, 6]，但绝大部分参数都落在了[-3, 2.1]的区间内；此时，为了能将参数均匀地分散到256个子区间，以获得更好的量化效果，可以将MAX的值从6调整为max{|-3|, 2.1}=3以截断[-3, 3]之外的参数，即将包含绝大部分参数的区间[-3, 3]线性映射到[-127, 127]，而区间(3, 6]内的参数则统一使用127来表示。

&emsp;&emsp;运用浮点数量化的方法，可以大大降低模型对存储和带宽的需求与压力，但在最后生成预测结果时，需要根据相应的量化方法，对量化后的定点数进行反量化/去量化（即还原成FP32浮点数），如图1-18所示。

<center><img src="../assets/1-18.png" width = 650></center>
<center>图1-18 量化(Quantize)与去量化(Dequantize)</center>

&emsp;&emsp;需要注意的是，量化只能减轻模型运行时对存储和带宽的依赖，并不能直接带来网络前向推导的加速效果，并且通常会造成模型预测精度的降低。
